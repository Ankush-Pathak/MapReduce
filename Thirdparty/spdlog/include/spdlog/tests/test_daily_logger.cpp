...
TEST_CASE("daily_logger with dateonly calculator", "[daily_logger]")
{
    ...
    spdlog::fmt_lib::format_to(
            std::back_inserter(w), SPDLOG_FILENAME_T("{}_{:04d}-{:02d}-{:02d}"), basename, tm.tm_year + 1900,
...
TEST_CASE("daily_logger with custom calculator", "[daily_logger]")
{
    ...
    filename_memory_buf_t w;
    spdlog::fmt_lib::format_to(
            std::back_inserter(w), SPDLOG_FILENAME_T("{}{:04d}{:02d}{:02d}"), basename, tm.tm_year + 1900,
            tm.tm_mon + 1, tm.tm_mday);
...
TEST_CASE("daily_file_sink::daily_filename_format_calculator", "[daily_file_sink]]")
{
    ...
    REQUIRE(filename ==
            spdlog::fmt_lib::format(SPDLOG_FILENAME_T("example-{:04d}-{:02d}-{:02d}.log"), tm.tm_year + 1900,
                                    tm.tm_mon + 1, tm.tm_mday));
}
